...
新视频首次运行会弹窗：先框选静止背景并回车，再按顺序点击蘑菇顶部轴心、帽沿左端、帽沿右端、底座前方地面点并回车。按 R 重选，Esc 取消。
...
背景窗口随拖动显示黄色选框、绿色有效特征点和红色排除区域，顶部显示点数。
少于 20 点时不能按 Enter 确认，直接重新拖框即可；建议选择能提供 50 点以上的固定墙面文字、窗框、接缝等。
检测在原视频分辨率进行，显示缩放不会改变阈值含义或保存的坐标。点数只代表首帧可跟踪纹理，不保证背景真的静止或全片都能跟踪。
避开人物、字幕、黑边、纯色或反光区域；“静止背景”允许随相机运动在图像中移动。

排除区域按当前帧计算，不再把整段视频的人体矩形框取并集。已有 ViTPose 关键点置信度足够时使用带留量的人体凸包及头部范围，避免抬腿导致矩形框误排除大片背景；关键点不可靠时保守退回当前人体框。
这是近似排除区域，不是精确人体分割，也不能自动识别画面中所有其他运动人物。
跟踪比较首帧直接匹配和相邻帧连续匹配，保留支持点更多且通过几何检查的结果；连续匹配始终携带原始参考点，不逐帧累乘相机旋转。
从参考帧分别向前、向后跟踪，输出参考帧的相机旋转为单位矩阵。默认 `--camera-tracking balanced`（适中模式）：每帧至少 10 个几何内点，二维匹配 RANSAC 容差 2.5 像素，旋转拟合中位误差不超过 3.5 像素。
适中模式同时要求内点比例至少 40%、内点的两个主方向均有至少 2 像素标准差，并限制每帧旋转拟合误差的 90 分位不超过 8 像素，防止少量聚集或偶然匹配通过。
仍建议围绕同一片固定墙面框选；不同选区可能给出不同旋转近似，尤其是把人物附近或不同深度的物体一起框入时。全片跟踪通过不等于完整三维相机运动已校准正确。
`--camera-tracking strict` 保留此前的 15 内点 / RANSAC 1.5 像素 / 旋转拟合中位误差 2 像素门槛，可用于对照。
首帧框选仍至少需要 20 个点；适中模式允许后续有部分点丢失，不要求用户精确复刻示例矩形。
失败信息分别报告原始跟踪点数、几何内点数和旋转拟合误差；几十个跟踪点不等于几十个可靠内点。
相邻跟踪仍可能缓慢漂移，这些检查不能证明完整三维相机运动正确；大幅平移、变焦或明显视差仍超出当前旋转相机模型。

统一入口在背景跟踪失败时显示原因并返回背景窗口，保留蘑菇四个关键点；不会默认为静止相机，也不会带着失败的相机数据继续优化。
缓存背景可通过 `--reselect-background` 单独重选，不必改参考帧或重标蘑菇。`--overwrite` 仍只表示允许覆盖结果，不表示重新标注。
例如重新框选 child_1 的背景并继续完整优化：

```powershell
python "D:\GitHub\GVHMR\tools\optimize_mushroom.py" `
  --input "D:\track_dataset\GVHMR_results\mushroom_child_1" `
  --start 1 --end 246 `
  --circle-start 92 --circle-end 223 `
  --reselect-background --overwrite
```

该视频首帧可参考右上方墙面区域 `(995, 5, 1280, 325)`，适中模式也已验证较宽的 `(975, 3, 1298, 531)`；这些像素坐标不能直接复用到其他视频。
从旧版升级后需先退出正在等待框选的旧 Python 进程再重跑，运行中的进程不会自动加载新代码；不必删除已保存的蘑菇关键点。
`--camera static` 不要求框选背景；`--reselect-background` 仅用于交互式 jitter 模式，不与 `--dry-run` 同用。
相机步骤仍输出 `camera_motion.npz`、`camera_motion.json`、`background_tracks.jpg`，其中预览包含点数和排除区域，JSON 记录实际跟踪模式及门槛，不新增大诊断文件。
单独调用 `tools/estimate_mushroom_camera.py` 时，模式参数为 `--tracking-mode balanced|strict`，默认同样为 balanced。

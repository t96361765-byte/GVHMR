...
def annotate(frame):
    ...
    import cv2
    scale = min(1., 1200 / frame.shape[1], 800 / frame.shape[0])
    ...
    try:
        while True:
            ...
            for i, (x, y) in enumerate(points):
                ...
                cv2.putText(view, str(i+1), pos, 0, .8, (0, 255, 0), 2)
            title = labels[len(points)] if len(points) < 4 else 'ENTER to accept'
            cv2.putText(view, title, (10, 30), 0, .8, (0, 255, 255), 2)
...
def main():
    ...
    p.add_argument('--bvh-range', nargs=2, type=int, default=[740, 1170], help='Stable circles in the BVH; change when using another BVH')
    ...
    p.add_argument('--iterations', type=int, default=1100)
    p.add_argument('--blender', default=DEFAULT_BLENDER)
    p.add_argument('--addon', default=DEFAULT_ADDON)
    p.add_argument('--overwrite', action='store_true')
    p.add_argument('--dry-run', action='store_true', help='Validate and print commands without writing or running optimization')
    args = p.parse_args()
    ...
    for path in [folder/'hmr4d_results.pt', folder/'preprocess/vitpose.pt', folder/'preprocess/bbx.pt',
                 folder/'0_input_video.mp4', Path(args.bvh), Path(args.blender), Path(args.addon)/'__init__.py']:
        if not path.is_file():
    ...
    elif folder == Path(r'D:\track_dataset\GVHMR_results\mushroom_child_8').resolve():
        cfg = json.loads((ROOT/'tools/configs/mushroom_child_8.json').read_text())
        cfg.update(camera_roi=[485, 10, 1430, 320], reference_frame=60)
    ...
    if not cfg.get('apparatus_pixels') or not cfg.get('camera_roi'):
        if args.dry_run:
            p.error('New video needs annotations. Run normally for interactive setup, or provide --config with apparatus_pixels and camera_roi.')
    ...
    if cfg.get('keep_range') != [args.start, args.end] or cfg.get('circle_range') != [args.circle_start, args.circle_end]:
        ...
        for key in ('evaluation_cycle_boundaries', 'extra_hand_contacts', 'ground_contacts', 'occluded_keypoints'):
            cfg.pop(key, None)
    cfg.update(keep_range=[args.start, args.end], circle_range=[args.circle_start, args.circle_end],
               fps=fps, reference_frame=reference, bvh_circle_range=args.bvh_range, source_folder=str(folder))
    camera = setup/'camera'
    ...
    if args.camera == 'jitter':
        run('estimate_mushroom_camera.py', '--input', folder, '--output', camera,
            '--roi', *cfg['camera_roi'], '--reference-frame', reference)
        camera_args = ['--camera-motion', camera/'camera_motion.npz']
    run('refine_mushroom.py', '--input', folder, '--bvh', args.bvh, '--config', config_path,
        '--output-root', output_root, '--iterations', args.iterations, *camera_args,
        *(['--overwrite'] if args.overwrite else []))
    run('preview_mushroom.py', '--input', output)
    run('audit_mushroom_mesh.py', '--input', output)
    execute([args.blender, '--background', '--factory-startup', '--python-exit-code', '1',
             '--python', str(ROOT/'tools/export_mushroom_blender.py'), '--', '--input', str(output), '--addon', args.addon])
